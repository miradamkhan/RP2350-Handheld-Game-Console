/*
 * tetris_game.h — Tetris game logic, fully separated from hardware.
 *
...
#define TETRO_SIZE       4   /* bounding box is 4x4 */
...
typedef struct {
    piece_type_t type;
    int8_t       rotation;  /* 0..3 */
    int8_t       x;         /* column of top-left of 4x4 box */
    int8_t       y;         /* row   of top-left of 4x4 box  */
} piece_t;
...
typedef enum {
    ...
    TETRIS_ACTION_LEFT    = (1 << 0),
    TETRIS_ACTION_RIGHT   = (1 << 1),
...
/* Advance one game tick.  `actions` is a bitmask of tetris_action_t.

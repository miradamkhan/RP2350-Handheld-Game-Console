 ...
 *
 * High score layout (at a given base address):
...
{
    uint8_t buf[6];
    if (!eeprom_read(addr, buf, 6)) return 0;
...
{
    uint32_t stored = eeprom_read_high_score(addr);
...
void eeprom_test(void)
{
    ...
    uint32_t hs = eeprom_read_high_score(EEPROM_ADDR_TETRIS_HS);
    printf("Stored Tetris high score: %lu\n", (unsigned long)hs);

    hs = eeprom_read_high_score(EEPROM_ADDR_FLAPPY_HS);
    printf("Stored Flappy high score: %lu\n", (unsigned long)hs);

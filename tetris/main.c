/*
 * main.c — Tetris for RP2350 Proton board.
 *
 * Ties together display, input, audio, EEPROM, and game logic.
 * Runs a ~30 FPS fixed-timestep loop.
 */
...
static game_state_t s_prev_state = STATE_MENU;
...
{
    char buf[12];
    /* Convert to string manually to avoid pulling in snprintf. */
    int i = 0;
    if (val == 0) {
        buf[i++] = '0';
    } else {
...
static uint8_t map_input(const input_state_t *in, game_state_t state)
{
    uint8_t actions = ACTION_NONE;
    ...
    if (dir == JOY_LEFT || dir == JOY_RIGHT) {
        ...
        if (fire) {
            actions |= (dir == JOY_LEFT) ? ACTION_LEFT : ACTION_RIGHT;
        }
    }
    ...
    if (in->button_pressed) {
        switch (state) {
        case STATE_MENU:      actions |= ACTION_SELECT; break;
        case STATE_PLAYING:   actions |= ACTION_ROTATE; break;
        case STATE_PAUSED:    actions |= ACTION_SELECT; break;
        case STATE_GAME_OVER: actions |= ACTION_SELECT; break;
        }
    }
...
static void draw_board(const game_t *g)
{
...
static void draw_info_panel(const game_t *g)
{
...
static void draw_menu(const game_t *g)
{
    display_fill(COLOR_BLACK);
    display_string(66, 60, "TETRIS", COLOR_CYAN, COLOR_BLACK, 3);
    display_string(30, 130, "Press Button", COLOR_WHITE, COLOR_BLACK, 2);
    display_string(48, 158, "to Start!", COLOR_WHITE, COLOR_BLACK, 2);

    display_string(INFO_X - 100, 230, "HIGH", COLOR_GRAY, COLOR_BLACK, 1);
    draw_number(INFO_X - 100, 242, g->high_score, COLOR_YELLOW, COLOR_BLACK, 2);
}

static void draw_playing_full(const game_t *g)
{
    display_fill(COLOR_BLACK);
    draw_board_border();
    draw_board(g);
...
static void draw_game_over(const game_t *g)
{
    display_fill(COLOR_BLACK);
...
static void handle_audio(const game_t *g)
{
    if (g->state_changed && g->state == STATE_GAME_OVER) {
        audio_play_sfx(SFX_GAME_OVER);
        return;
    }
    if (g->state_changed && g->state == STATE_PLAYING) {
...
int main(void)
{
    ...
    /*
     * Uncomment one or more of these to run hardware tests at boot:
     *   display_test();
     *   input_test();
     *   audio_test();
     *   eeprom_test();
    ...
    uint32_t high = eeprom_read_high_score();
    ...
    while (true) {
        ...
        game_state_t cur = game_tick(&g, actions);

        /* 3. Render based on state transitions and dirty flags */
        if (g.state_changed) {
            switch (cur) {
            case STATE_MENU:      draw_menu(&g);           break;
            case STATE_PLAYING:   draw_playing_full(&g);   break;
            case STATE_PAUSED:    draw_paused();           break;
            case STATE_GAME_OVER:
                eeprom_write_high_score(g.score);
                draw_game_over(&g);
                break;
            }
        } else if (cur == STATE_PLAYING) {
